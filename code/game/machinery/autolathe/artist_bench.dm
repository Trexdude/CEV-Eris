...
/obj/machinery/autolathe/artist_bench/Topic(var/datum/mind/player, var/mob/living/carbon/human/H, href, href_list)
	if(..())
		return
	...
	if(href_list["create_art"])
		var/ins_used = null
		if(player.assigned_role == "Actor") //Temporary job title
...
/obj/machinery/autolathe/artist_bench/proc/create_art(mob/living/user, var/mob/living/carbon/human/H, var/datum/mind/player, ins_used)
	if(!ins_used)
		return
	...
	if(player.assigned_role == "Actor")
		if(strange_item)
			var/datum/component/inspiration/I = strange_item.GetComponent(/datum/component/inspiration)
			var/list/L = I.calculate_statistics()
			for(var/stat in L)
				var/stat_up = L[stat]
				to_chat(user, SPAN_NOTICE("Your [stat] stat goes up by [stat_up]."))//Temporary decsription
				H.stats.changeStat(stat, stat_up)
